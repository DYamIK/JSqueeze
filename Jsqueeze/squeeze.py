...
import numpy as np
from Jsqueeze.anomaly_amount_fileter import KPIFilter
from Jsqueeze.squeeze_option import SqueezeOption
from Jsqueeze.clustering import cluster_factory
    ...
    def _locate_in_cuboid(self, cuboid, indices, **params) -> Tuple[FrozenSet[AC], float]:
        ...
        def _root_cause_score(partition: int) -> float:
            ...
            data_p, data_n = self.get_derived_dataframe(
                frozenset(elements[(partition-1):partition]), cuboid=cuboid,
            ...
            log_ll = np.mean(norm.pdf(dp, loc=0.1, scale=0)) \
            ...
            _lp = len(data_p)
            # M1,M2=(_v1+_f2)*0.5,(_v2+_f2)*0.5
            # _f1=0.5*entropy(_v1.astype(float),M1.astype(float))+ 0.5*entropy(_f1.astype(float),M1.astype(float))
            # _v1=0.5*entropy(_v2.astype(float),M2.astype(float))+ 0.5*entropy(_f2.astype(float),M2.astype(float))
            ...
            divide = lambda x, y, z: (x+z) / y if y > 0 else (0 if x == 0 else float('inf'))
            ...
            _ps = 1 - (divide(dis_f(_v1, _a1), len(_v1), 0)) / (divide(dis_f(_v1, _f1), len(_v1), 0))
            # M_1=Js_div(_v1,_a1)
            # M_2=Js_div(_v2,_f2)
            # M_3=Js_div(_v1,_f1)
            # M_4=Js_div(_v2,_f2)
