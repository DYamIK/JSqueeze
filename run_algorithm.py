...
from Jsqueeze import Squeeze, SqueezeOption
import pandas as pd
...
@click.option("--name", default="B_cuboid_layer_3_n_ele_3", help="name of this setting")
@click.option("--input-path", default="B5", help="will read data from {input_path}/{name}")
@click.option("--output-path", default="output/B5", help="if {output_path} is a dir, save to {output_path}/{name}.json; \
otherwise save to {output_path}")
@click.option("--num-workers", default=10, help="num of processes")
...
def main(name, input_path, output_path, num_workers, **kwargs):
    ...
    # injection_info = pd.read_csv(f'{input_path}/{name}/ injection_info.csv', engine='c')
